...
export type JoinVideoSourceType = 'file' | 'camera'

interface JoinVideoSourceSelectionProps {
    onSelect: (source: JoinVideoSourceType) => void
}

export function JoinVideoSourceSelection({ onSelect }: JoinVideoSourceSelectionProps) {
    const [hoveredSource, setHoveredSource] = React.useState<JoinVideoSourceType | null>(null)
            ...
            >
                ...
                >
                    ...
                    <TerminalText className="text-sm text-white/50 tracking-wide">
                        Select how to view the broadcast video
                    </TerminalText>
                </motion.div>

                {/* Selection Cards */}
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6 md:gap-8">
                    {/* Camera Card - for listening to live camera */}
                    <motion.button
                        initial={{ opacity: 0, y: 20 }}
                        animate={{ opacity: 1, y: 0 }}
                        transition={{ duration: 0.5 }}
                    ...
                    {/* Watch File Card - for watching uploaded file */}
                    <motion.button
                        initial={{ opacity: 0, y: 20 }}
                        animate={{ opacity: 1, y: 0 }}
                        transition={{ duration: 0.5 }}
                ...
                >
                    <TerminalText className="text-xs text-white/30 tracking-widest">
                        SELECT INPUT METHOD TO INITIALIZE VIDEO STREAM
                    </TerminalText>
                </motion.div>
            </motion.div>
        </motion.div>
